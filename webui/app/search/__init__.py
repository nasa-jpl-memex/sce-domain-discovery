...
def query_and_fetch(query, model, top_n=12, page=1):
                ...
                while result_size > 0 and len(url_details) < top_n:
                    ...
                    for fetched_data in fetched_result:
                        try:
                            ...
                            try:
                                #app.logger.info("http://localhost:8050/render.png?url="+fetched_data[0]+"&width=320&height=240")
    ...
    try:
        model = models[model]
        model['url_text'] = url_text.encode('utf-8').strip()
        model['url_details'] = url_details.encode('utf-8').strip()
        model.save()
